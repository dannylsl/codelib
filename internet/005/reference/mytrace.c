/* This is the simplest reference of homework4.
   Maybe it is not good, but enough for our requirement.
...
int main (int argc, char *argv[])
{
   ...
   printf("traceroute to %s (%s), %d hops max, %d byte packets\n",
	   argv[1], p, MAX_HOPS, sizeof(struct iphdr) + sizeof(struct
                udphdr));
             ...
             struct timeval *tv)
{
   ...
   for(;;) {
     ...
     recv_len =recvfrom(sockfd,recv_buf,BUF_LEN,0,
               (struct sockaddr *)hopaddr,
                &socklen);
