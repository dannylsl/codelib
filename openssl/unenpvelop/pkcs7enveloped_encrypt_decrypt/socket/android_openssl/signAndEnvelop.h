 ...
 * @Param: outfilename	输出数字签名信封文件名
 */
char *ssl_PKCS7_signed_and_enveloped_from_file(EVP_PKEY *prikey, STACK_OF(X509)*ca,	X509* x509_Cert , const char* filename, const int fileCodeType, const char* datafilename,const char* outfilename)
{
...
//	PKCS7_set_detached(p7,0);
	...
	//将DER编码后的内容写入文件
	write_data_to_file(der,derLen,outfilename);
