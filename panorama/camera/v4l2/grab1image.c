...
static int read_frame (void)
{
	...
	unsigned int i;
	FILE *bmpHeader;
	char charBmpHeader[BMPHEADER_LEN];

	if(NULL==(bmpHeader = fopen("bmpheader320_240","rb+"))){
		perror("BMPHEADER OPEN FAILED");
		exit(0);
	}
	fread(charBmpHeader,BMPHEADER_LEN,1,bmpHeader);
	fwrite(charBmpHeader,BMPHEADER_LEN,1,file_fd);
	fclose(bmpHeader);
	CLEAR (buf);
...
int main (int argc,char ** argv)
{
	...
	remove("yuyv.bmp");
	file_fd = fopen("yuyv.bmp", "w");//图片文件名

	fd = open (dev_name, O_RDWR /* required */ | O_NONBLOCK, 0);//打开设备

	if(ioctl (fd, VIDIOC_QUERYCAP, &cap)){//获取摄像头参数
...
//	fmt.fmt.pix.pixelformat = V4L2_PIX_FMT_MJPEG;
	...
	for (i = 0; i < n_buffers; ++i)  //将缓冲帧放入队列 
	{
		struct v4l2_buffer buf;
		CLEAR (buf);
