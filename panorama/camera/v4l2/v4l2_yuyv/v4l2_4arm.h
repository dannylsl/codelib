...
#ifndef _V4L2_4ARM_H_
#define _V4L2_4ARM_H_
...
typedef struct _v4l2_struct
{
	...
	unsigned char data[WIDTH*HEIGHT*PIXEL];
}v4l2_device;
...
int v4l2_set_capture_fmt(v4l2_device*vd){
	...
	vd->fmt.fmt.pix.height		= WIDTH;
	vd->fmt.fmt.pix.width		= HEIGHT;
...
void v4l2_start_capture(v4l2_device*vd){
	int i;
	struct v4l2_buffer buf;
	enum v4l2_buf_type type;
...
void v4l2_stop_capture(v4l2_device*vd)
{
	enum v4l2_buf_type type;
	type = V4L2_BUF_TYPE_VIDEO_CAPTURE;

	if (ioctl(vd->fd, VIDIOC_STREAMOFF, &type) == -1)
	{
...
int v4l2_read_frame(v4l2_device*vd){
	...
	//memcpy(&vd->data,vd->buffers[vd->buf.index].start,WIDTH*HEIGHT*PIXEL);	
...
int v4l2_get_frame_size(v4l2_device*vd){
	return vd->buffers[vd->buf.index].length;	
}

//Get frame buffer data and Upsidedown
unsigned char* v4l2_get_frame_data(v4l2_device*vd){
	int i,j;
	int height = vd->height;
	int width  = vd->width;
	unsigned len = v4l2_get_frame_size(vd);
	unsigned char* start = (unsigned char*)vd->buffers[vd->buf.index].start;	

	for(i = 0; i < height;i++){
		for(j = 0; j < width*2; j++){
			vd->data[(height-1-i)*width*2+j]=*(start+i*width*2+j);		
		}
	}
	return vd->buffers[vd->buf.index].start;	
}
